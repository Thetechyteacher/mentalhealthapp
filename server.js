...
app.get('/',async (request, response)=>{
    const todoItems = await db.collection('psicohelp').find().toArray()//go get the collection to do and convert it to an array
    const itemsLeft = await db.collection('psicohelp').countDocuments({completed: false})// get the docs that have the property completed false (the ones left to do)
    ...
    // db.collection('psicohelp').find().toArray()
    // .then(data => {
    //     db.collection('psicohelp').countDocuments({completed: false})
    //     .then(itemsLeft => {
...
app.post('/addTodo', (request, response) => { // get the info from /todo form and post it
    db.collection('psicohelp').insertOne({thing: request.body.todoItem, completed: false})// find the collection todo, insert into the property thing whatever is in the input to-do item
    .then(result => {
...
app.put('/markComplete', (request, response) => {// get the function from the js client side
    db.collection('psicohelp').updateOne({thing: request.body.itemFromJS},{// find this collection, update one element, insert whatever the body from itemFromJs contains into the property thing.
...
app.delete('/deleteItem', (request, response) => {
    db.collection('psicohelp').deleteOne({thing: request.body.itemFromJS})// get the text from itemFromJs
    .then(result => {
